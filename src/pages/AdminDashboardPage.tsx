...
export function AdminDashboardPage() {
  ...
  if (stats.role === 'admin' || stats.role === 'developer') {
    ...
    const layoutTitles = {
      ...
      v1: { title: '대시보드', subtitle: '전체 현황을 한눈에 확인하세요' },
      v2: { title: '대시보드 V2 - 3단 컬럼', subtitle: '전체통계 | 작업현황 | 최근활동' },
      v3: { title: '대시보드 V3 - 카드 강조', subtitle: '큰 통계 카드 + 그리드 레이아웃' }
    };

    const currentLayout = layoutTitles[layout as keyof typeof layoutTitles] || layoutTitles.v1;
    ...
    return (
      <div>
        ...
        {layout === 'v2' && (
          <div>
            <p className="text-gray-500">V2 레이아웃은 추후 구현 예정입니다.</p>
          </div>
        )}
